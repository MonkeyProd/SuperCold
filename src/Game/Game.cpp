...
void Game::shootBullet(sf::Vector2f velocity) {
  bulletLayer.push_back(Bullet{{player.get_playerObject().getPosition().x,
...
void Game::ProcessEvents() {
  ...
  while (window.pollEvent(event)) {
    if (not paused) {
      ...
      if (Up) shootBullet({0, -200});
      if (Down) shootBullet({0, 200});
      ...
      if (not(A or D)) player.resetHorizontalVelocity();
